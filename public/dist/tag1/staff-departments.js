...
self.on("mount", function () {
  self.title = 'Create';
  self.update();
  self.readStaffDepartments();
});

self.on("unmount", function () {
  staffdepartmentStore.off('staff_departments_changed', DepartmentsChanged);
});
self.readStaffDepartments = function () {
  staffdepartmentStore.trigger('read_staff_departments');
};

self.add = function () {
  if (!self.refs.addStaffDepartmentInput.value) {
    toastr.info("Please enter Department and try again");
  } else if (!self.refs.addstaffDepartmentFullNameInput.value) {
    toastr.info("Please enter Full Name and try again");
  } else {
    self.loading = true;
    if (self.title == 'Create') {
      console.log('create');
      staffdepartmentStore.trigger('add_staff_department', self.refs.addStaffDepartmentInput.value, self.refs.addstaffDepartmentFullNameInput.value);
    } else if (self.title == 'Update') {
      console.log('update');
      staffdepartmentStore.trigger('edit_staff_department', self.refs.addStaffDepartmentInput.value, self.refs.addstaffDepartmentFullNameInput.value, self.edit_id);
    }
  }
};
...
self.delete = function (e) {
  self.loading = true;
  staffdepartmentStore.trigger('delete_staff_department', e.item.d.id);
};

self.edit = function (d, e) {
  console.log(d);
  self.title = 'Update';
  self.refs.addStaffDepartmentInput.value = d.department;
  self.refs.addstaffDepartmentFullNameInput.value = d.department_full_name;
  self.edit_id = d.id;
};

staffdepartmentStore.on('staff_departments_changed', DepartmentsChanged);
function DepartmentsChanged(departments) {
  console.log('departments_changed1');
  console.log(departments);
  self.title = 'Create';
  self.refs.addStaffDepartmentInput.value = '';
  self.refs.addstaffDepartmentFullNameInput.value = '';
  self.loading = false;
