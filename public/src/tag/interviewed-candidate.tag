<interviewed-candidate>
	...
	<table class="table is-fullwidth is-bordered is-hoverable is-narrow">
		...
		<tbody>
			<tr each={a, i in InterviewedCandidateData}>
				<td>{ i+1 }</td>
				<td>{a.career_id}</td>
				<td>{a.full_name}</td>
	...
	<!-- Open Cancel Result Modal Start -->
  	<div id="cancelresultModal" class="modal ">
      		...
      		<header class="modal-card-head">
        		<p class="modal-card-title">Confirmation Required</p>
		      	...
		      	<section class="modal-card-body">
		        	<div class="columns">
		          		<div class="column">
		          			<h1 class="title has-text-centered is-size-5" style="color: #ff3860;">Are you sure you want to Cancel the Interview Status of the selected candidate?</h1>
	    				</div>
		        	</div>
		        </section>
	      	<footer class="modal-card-foot">
	        	<button class="button is-success" onclick={UpdateInterviewedCandidate} >Yes</button>
	        	<button class="button is-danger" id="item-modal-close" 
	        	onclick={closecancelresultModal}>No</button>
	      	</footer>
    	</div>
  	</div>
