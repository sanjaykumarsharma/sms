		...
		<table class="table is-fullwidth is-striped is-hoverable is-bordered" >
			...
			<tbody>
				<tr each={cd, i in collectionSummary}>
					<td class="{cd.class_hide}">{i + 1}</td>
					<td  class="{cd.class}">{cd.bank}</td>
					<td class="has-text-right amount {cd.class}">{cd.Cash}</td>
					<td class="has-text-right amount {cd.class}">{cd.Cheque}</td>
					<td class="has-text-right amount {cd.class}">{cd.Online}</td>
					<td class="has-text-right amount {cd.class}">{cd.total}</td>
				</tr>
				<!-- <tr>
    ...
    function ReadCollectionSummaryChanged(collectionSummary, session_name){
       console.log(collectionSummary) 
          ...
          self.collectionSummary.map(c => {
            ...
            if(c.online) self.totalOnline =Number(self.totalOnline) + Number(c.Online)*/
      })
