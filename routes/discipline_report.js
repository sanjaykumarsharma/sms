...
router.get('/csv_export_read_case_wise_report/:start_date/:end_date', function(req, res, next) {
              ...
              const csv = json2csvParser.parse(result);
                ...
                fs.writeFile(path, csv, function(err,data) {
                  if (err) {throw err;}
                    ...
                    res.send(data)
                    var url='http://localhost:4000/csv/disicplineCategoyReport.csv';
...
router.get('/csv_export_read_date_wise_case_report/:start_date/:end_date/:category_id', function(req, res, next) {
  ...
  req.getConnection(function(err,connection){
     ...
     var qry = `select a.enroll_number as 'Enroll No', concat(first_name,' ',middle_name,'',last_name)as 'Name',
                concat(standard,' ',section)as 'Class', g.case_name as 'Case',
                date_format(consult_date,'%d/%m/%Y')as 'Date', diagnosis as 'Diagnosis', remarks as 'Remarks'
                from discipline a
     ...
     connection.query(qry, function(err, result)     
     {
        ...
        if(err){
           ...
           data.status = 'e';
            ...
            const fields = ['Name','Enroll No','Class','Case','Date','Diagnosis','Remarks'];
              const json2csvParser = new Json2csvParser({ fields });
              const csv = json2csvParser.parse(result);
                ...
                fs.writeFile(path, csv, function(err,data) {
                  if (err) {throw err;}
                    ...
                    res.send(data)
                    var url='http://localhost:4000/csv/disicplineDateWiseCaseReport.csv';
                    var open = require("open","");
                    open(url);  
                  }
                });
...
router.get('/csv_export_read_class_wise_report/:standard_id/:section_id/:session_id', function(req, res, next) {
  ...
  req.getConnection(function(err,connection){
    ...
    var data = {}
     ...
     var qry = `select category_name as 'Case', count(*) as total
                from discipline a
              ...
              const csv = json2csvParser.parse(result);
                ...
                fs.writeFile(path, csv, function(err,data) {
                  if (err) {throw err;}
                    ...
                    res.send(data)
                    var url='http://localhost:4000/csv/disicplineClassWiseReport.csv';
                    var open = require("open","");
                    open(url);  
                  }
                });
