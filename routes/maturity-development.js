...
router.get('/students/:section_id/:exam_term', function(req, res, next) {
  ...
  req.getConnection(function(err,connection){
     ...
     var qry = `select a.student_id, concat(standard,'-',section) as standard, 
                ...
                Left join  student_maturity_development e on (a.student_id = e.student_id and e.session_id= ${req.cookies.session_id} and e.exam_term='${req.params.exam_term}')
     ...
     connection.query(qry,function(err,result)     {
        ...
        if(err){
           ...
           data.status = 'e';
            ...
            data.status = 's';
            data.maturityDevelopments = result;
            res.send(JSON.stringify(data))
        }
...
// read for update maturity development
router.get('/read-for-update/:student_id/:exam_term', function(req, res, next) {
